...
class login extends JFrame implements ActionListener
{
	...
	login()
	{
		...
		b1.setBounds(220,40,80,30);
		b1.addActionListener(this);
		b2=new JButton("CANCEL");
		b2.setBounds(50,130,80,30);
		b2.addActionListener(this);
		b3=new JButton("SIGNUP");
		b3.setBounds(220,80,80,30);
		b3.addActionListener(this);
...
class sign extends JFrame implements ActionListener
{
	...
	sign()
	{
		...
		//getContentPane().setBackground(Color.green);
		getContentPane().setBackground(new java.awt.Color(216,191,216));
...
class bank_p extends JFrame implements ActionListener
{
	...
	bank_p()
	{
		...
		getContentPane().setBackground(new java.awt.Color(238,232,170));
		...
		l2.setFont(f2);
		l2.setBounds(1000,105,500,100);
		Font f4=new Font("Aharoni",Font.ITALIC,30);
		l3=new JLabel("SUBMITTED BY:naman,navneet");
		l3.setFont(f4);
		l3.setBounds(30,550,500,100);
		l4=new JLabel("mukesh,navjot");
		l4.setFont(f4);
		l4.setBounds(100,600,500,100);
		l5=new JLabel("B.TECH Computer Science");
		l5.setFont(f4);
		l5.setBounds(30,650,600,100);
		l6=new JLabel("CHITKARA COLLEGE");
		Font f3=new Font("Book Antiqua",Font.ITALIC,30);
		l6.setFont(f3);
		l6.setBounds(400,230,600,30);
		l7=new JLabel("PUNJAB,RAJPURA");
		l7.setFont(f3);
		l7.setBounds(500,270,500,30);
		ImageIcon i1=new ImageIcon("chitkara.png");
		Border border=BorderFactory.createRaisedBevelBorder();
		Border tripleBorder=new CompoundBorder(new CompoundBorder(border,border),border);
		UIManager.put("Button.border",tripleBorder);
...
class bank_project1 extends JFrame implements ActionListener
{
	...
	bank_project1()
	{
		...
		ImageIcon i1=new ImageIcon("management2.jpg");
		l2=new JLabel(i1);
		...
		try
		{
			...
			if(e1.getSource()==b1)
			{
				...
				else
				{
					...
					int count=0;
					String s[]={"Date","AccNo","Type","Amount"};
		...
		try
		{
			...
			if(e1.getSource()==b1)
			{
				...
				else
				{
					...
					if(bal<amt)
					{
						JOptionPane.showMessageDialog(this,"You don't have sufficient balance in your account");
						JOptionPane.showMessageDialog(this,"You have Rs."+bal);
						tf1.setText("");
						tf2.setText("");
						tf3.setText("");
					}
...
class create extends JFrame implements ActionListener
{
	...
	
	{
		try
		{
			...
			if(e1.getSource()==b2)
			{
				if((tf1.getText().equals(""))||(tf2.getText().equals(""))||(ta1.getText().equals(""))||(tf3.getText().equals(""))||(tf4.getText().equals(""))||(tf5.getText().equals(""))||(tf6.getText().equals(""))||(tf7.getText().equals(""))||(tf8.getText().equals("")))
				{
				...
				{
					ps=con.prepareStatement("select*from bank_table");
					ResultSet rs=ps.executeQuery();
					while(rs.next())
					{
						accno=rs.getInt(1);
					...
					ps=con.prepareStatement("insert into bank_table values(?,?,?,?,?,?,?,?,?,?,?)");
					ps.setInt(1,accno);
					...
					ps.setString(9,tf7.getText());
					ps.setInt(10,Integer.parseInt(tf8.getText()));
...
class trans extends JFrame implements ActionListener
{
	...
	trans()
	{
		...
		setVisible(true);
		//getContentPane().setBackground(Color.blue);
		getContentPane().setBackground(new java.awt.Color(220,220,220));
		Color c1=new Color(0,0,0);
